function fmdl=GetReconMesh(R,H, calc_new, shape, Bh)
...
%     extra={'ball',heart_object_str};
    ...
    elec_shape = [0.0254,0.0254,R*0.04];  % point electrode configuration

    % elec_shape=[0,0,0.01];

    switch shape
        case "c"
            cyl_shape=[R, [H, [R*0.04]]];   % (height, [cylinder radius, max elem size])
