function [Q0_new, multi_Q0, errs]=FindBestQ0(Q0, Bh, elec_pts, model_info, Vd, method, fwd_map, construct_map, num_src, file_ext)
...
switch method
    ...
    case "FitFwdSims"
        ...
        fmdl=GetReconMesh(R,H, false, model_info.shape);  Pt=fmdl.nodes;

        %Initial Guess
        theta_l13=(2*pi/16)*13; x13=-(model_info.R/2)*cos(theta_l13);   y13=-(model_info.R/2)*sin(theta_l13); z13=model_info.H/2;
        x0=[x13, y13, z13];

%         heart_pts=GetHeartRegionFEMMesh(Pt, Bh(4), x0);
        [heart_pts, ~]=GenerateSourceOnSphere(Bh(4), Bh(1:3), 250);
        % heart_pts(end+1,:)=x0;

%         heart_pts=Pt(setxor(1:N, fmdl.boundary),:);
        numHpts=length(heart_pts);
%         heart_pts(numHpts+1,:)=Qd;            numHpts=length(heart_pts);

        %             heart_ind=1:length(Pt);
            ...
            case "FEM"
                ...
                if construct_map
                    ...
                    for k=1:numHpts
                        ...
                        for i=1:3
                            b=CalculateRHSvec(rhs_setting, T, Pt, Qk, sigma_Omega, C, vol_tetr, fmdl, E(:,i)');  b(N+1)=0;
                    ...
                    save(file_ext+"BestFit_procG.mat", 'G');
                else
                    % load("BestQ0"+"BestFit_procG.mat");
